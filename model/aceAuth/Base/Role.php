...
use aceAuth\Group as ChildGroup;
use aceAuth\GroupQuery as ChildGroupQuery;
use aceAuth\Role as ChildRole;
use aceAuth\RoleQuery as ChildRoleQuery;
use aceAuth\User as ChildUser;
 ...
 * 
 *
* @package    propel.generator.aceAuth.Base
*/
abstract class Role implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\aceAuth\\Map\\RoleTableMap';
    ...
    /**
     * @var ObjectCombinationCollection Cross CombinationCollection to store aggregation of ChildGroup, ChildUser combination combinations.
     */
    protected $combinationCollGroupUsers;

    /**
     * @var bool
     */
    protected $combinationCollGroupUsersPartial;
     ...
     * @var ObjectCombinationCollection Cross CombinationCollection to store aggregation of ChildGroup, ChildUser combination combinations.
     */
    protected $combinationCollGroupUsersScheduledForDeletion = null;
    ...
    /**
     * Initializes internal state of aceAuth\Base\Role object.
     */
     ...
     * @return $this|\aceAuth\Role The current object (for fluent API support)
     */
    ...
    public function setGroupid($v)
    {
        ...
        if ($this->groupid !== $v) {
            $this->groupid = $v;
            $this->modifiedColumns[RoleTableMap::COL_GROUPID] = true;
        }

        if ($this->aroleInGroup !== null && $this->aroleInGroup->getId() !== $v) {
            $this->aroleInGroup = null;
        }
    ...
    public function setRole($v)
    {
        ...
        if ($this->role !== $v) {
            $this->role = $v;
            $this->modifiedColumns[RoleTableMap::COL_ROLE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : RoleTableMap::translateFieldName('Groupid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->groupid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : RoleTableMap::translateFieldName('Role', TableMap::TYPE_PHPNAME, $indexType)];
            $this->role = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : RoleTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\aceAuth\\Role'), 0, $e);
        }
    }
    ...
    {
        if ($this->aroleInGroup !== null && $this->groupid !== $this->aroleInGroup->getId()) {
            $this->aroleInGroup = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aroleInGroup !== null) {
                if ($this->aroleInGroup->isModified() || $this->aroleInGroup->isNew()) {
                    $affectedRows += $this->aroleInGroup->save($con);
                }
                $this->setroleInGroup($this->aroleInGroup);
            }
            ...
            if ($this->combinationCollGroupUsersScheduledForDeletion !== null) {
                if (!$this->combinationCollGroupUsersScheduledForDeletion->isEmpty()) {
                    $pks = array();
                    foreach ($this->combinationCollGroupUsersScheduledForDeletion as $combination) {
                        $entryPk = [];
                    ...
                    \aceAuth\UserGroupQuery::create()
                        ->filterByPrimaryKeys($pks)
                        ->delete($con);

                    $this->combinationCollGroupUsersScheduledForDeletion = null;
                }

            }

            if (null !== $this->combinationCollGroupUsers) {
                foreach ($this->combinationCollGroupUsers as $combination) {

                    //$combination[0] = Group (Groups_FK)
                    if (!$combination[0]->isDeleted() && ($combination[0]->isNew() || $combination[0]->isModified())) {
                        $combination[0]->save($con);
                    }
                
                    //$combination[1] = User (Users_FK)
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(RoleTableMap::COL_GROUPID)) {
            $modifiedColumns[':p' . $index++]  = 'groupId';
        }
        if ($this->isColumnModified(RoleTableMap::COL_ROLE)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aroleInGroup) {
                ...
                $result[$key] = $this->aroleInGroup->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\aceAuth\Role The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(RoleTableMap::COL_GROUPID)) {
            $criteria->add(RoleTableMap::COL_GROUPID, $this->groupid);
        }
        if ($this->isColumnModified(RoleTableMap::COL_ROLE)) {
            $criteria->add(RoleTableMap::COL_ROLE, $this->role);
        }
        if ($this->isColumnModified(RoleTableMap::COL_DESCRIPTION)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \aceAuth\Role (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \aceAuth\Role Clone of current object.
     * @throws PropelException
     */
     ...
     * @throws PropelException
     */
    public function setroleInGroup(ChildGroup $v = null)
    {
        if ($v === null) {
     ...
     * @throws PropelException
     */
    public function getroleInGroup(ConnectionInterface $con = null)
    {
        if ($this->aroleInGroup === null && ($this->groupid !== null)) {
            $this->aroleInGroup = ChildGroupQuery::create()->findPk($this->groupid, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aroleInGroup->addGroupRoles($this);
             */
        }

        return $this->aroleInGroup;
    }
    ...
    public function initUserRoles($overrideExisting = true)
    {
        ...
        $this->collUserRoles->setModel('\aceAuth\UserGroup');
    }
    ...
    public function getUserRoles(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collUserRoles || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collUserRoles) {
                ...
                $collUserRoles = ChildUserGroupQuery::create(null, $criteria)
                    ->filterByGroupRole($this)
                    ->find($con);
    ...
    {
        /** @var ChildUserGroup[] $userRolesToDelete */
        ...
        foreach ($userRolesToDelete as $userRoleRemoved) {
            $userRoleRemoved->setGroupRole(null);
        }
     ...
     * @return $this|\aceAuth\Role The current object (for fluent API support)
     */
    public function addUserRole(ChildUserGroup $l)
    {
        if ($this->collUserRoles === null) {
            $this->initUserRoles();
            $this->collUserRolesPartial = true;
    ...
    /**
     * @param ChildUserGroup $userRole The ChildUserGroup object to add.
     */
    protected function doAddUserRole(ChildUserGroup $userRole)
    {
        $this->collUserRoles[]= $userRole;
        $userRole->setGroupRole($this);
    }

    /**
     * @param  ChildUserGroup $userRole The ChildUserGroup object to remove.
     * @return $this|ChildRole The current object (for fluent API support)
     */
    public function removeUserRole(ChildUserGroup $userRole)
    {
     ...
     * @return ObjectCollection|ChildUserGroup[] List of ChildUserGroup objects
     */
    public function getUserRolesJoinGroup(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildUserGroupQuery::create(null, $criteria);
    ...
    /**
     * Initializes the combinationCollGroupUsers crossRef collection.
     *
     * By default this just sets the combinationCollGroupUsers collection to an empty collection (like clearGroupUsers());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @return void
     */
    public function initGroupUsers()
    {
        $this->combinationCollGroupUsers = new ObjectCombinationCollection();
        $this->combinationCollGroupUsersPartial = true;

    }

    /**
     * Checks if the combinationCollGroupUsers collection is loaded.
     *
     * @return bool
     */
    public function isGroupUsersLoaded()
    {
        return null !== $this->combinationCollGroupUsers;
    }

    /**
     * Gets a combined collection of ChildGroup, ChildUser objects related by a many-to-many relationship
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return ObjectCombinationCollection Combination list of ChildGroup, ChildUser objects
     */
    public function getGroupUsers($criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->combinationCollGroupUsersPartial && !$this->isNew();
        if (null === $this->combinationCollGroupUsers || null !== $criteria || $partial) {
            if ($this->isNew()) {
                // return empty collection
                if (null === $this->combinationCollGroupUsers) {
                    $this->initGroupUsers();
                ...
                $combinationCollGroupUsers = new ObjectCombinationCollection();
                foreach ($items as $item) {
                    $combination = [];

                    $combination[] = $item->getGroup();
                    $combination[] = $item->getUser();
                    $combinationCollGroupUsers[] = $combination;
                }

                if (null !== $criteria) {
                    return $combinationCollGroupUsers;
                }

                if ($partial && $this->combinationCollGroupUsers) {
                    //make sure that already added objects gets added to the list of the database.
                    foreach ($this->combinationCollGroupUsers as $obj) {
                        if (!call_user_func_array([$combinationCollGroupUsers, 'contains'], $obj)) {
                            $combinationCollGroupUsers[] = $obj;
                        }
                    }
                }

                $this->combinationCollGroupUsers = $combinationCollGroupUsers;
                $this->combinationCollGroupUsersPartial = false;
            }
        }

        return $this->combinationCollGroupUsers;
    }

    /**
     * Returns a not cached ObjectCollection of ChildGroup objects. This will hit always the databases.
     * If you have attached new ChildGroup object to this object you need to call `save` first to get
     * the correct return value. Use getGroupUsers() to get the current internal state.
     * 
     * @param ChildUser $user
     * @param Criteria $criteria
     * @param ConnectionInterface $con
     *
     * @return ChildGroup[]|ObjectCollection
     */
    public function getGroups(ChildUser $user = null, Criteria $criteria = null, ConnectionInterface $con = null)
    {
        return $this->createGroupsQuery($user, $criteria)->find($con);
    }

    /**
     * Sets a collection of ChildGroup, ChildUser combination objects related by a many-to-many relationship
     * to the current object by way of the usergroup cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
    ...
    public function setGroupUsers(Collection $groupUsers, ConnectionInterface $con = null)
    {
        ...
        $currentGroupUsers = $this->getGroupUsers();

        $combinationCollGroupUsersScheduledForDeletion = $currentGroupUsers->diff($groupUsers);

        foreach ($combinationCollGroupUsersScheduledForDeletion as $toDelete) {
            call_user_func_array([$this, 'removeGroupUser'], $toDelete);
        }

        foreach ($groupUsers as $groupUser) {
            if (!call_user_func_array([$currentGroupUsers, 'contains'], $groupUser)) {
                call_user_func_array([$this, 'doAddGroupUser'], $groupUser);
            }
        }

        $this->combinationCollGroupUsersPartial = false;
        $this->combinationCollGroupUsers = $groupUsers;

        return $this;
    }

    /**
     * Gets the number of ChildGroup, ChildUser combination objects related by a many-to-many relationship
     * to the current object by way of the usergroup cross-reference table.
     *
     * @param      Criteria $criteria Optional query object to filter the query
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return int the number of related ChildGroup, ChildUser combination objects
     */
    public function countGroupUsers(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->combinationCollGroupUsersPartial && !$this->isNew();
        if (null === $this->combinationCollGroupUsers || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->combinationCollGroupUsers) {
                return 0;
            } else {
            ...
            return count($this->combinationCollGroupUsers);
        }
    }

    /**
     * Returns the not cached count of ChildGroup objects. This will hit always the databases.
     * If you have attached new ChildGroup object to this object you need to call `save` first to get
     * the correct return value. Use getGroupUsers() to get the current internal state.
     * 
     * @param ChildUser $user
     ...
     * @return integer
     */
    public function countGroups(ChildUser $user = null, Criteria $criteria = null, ConnectionInterface $con = null)
    {
        return $this->createGroupsQuery($user, $criteria)->count($con);
    }
     ...
     */
    public function addGroup(ChildGroup $group, ChildUser $user)
    {
        if ($this->combinationCollGroupUsers === null) {
            $this->initGroupUsers();
        }

        if (!$this->getGroupUsers()->contains($group, $user)) {
            // only add it if the **same** object is not already associated
            $this->combinationCollGroupUsers->push($group, $user);
            $this->doAddGroupUser($group, $user);
        }
     ...
     * @return ChildRole The current object (for fluent API support)
     */
    public function addUser(ChildUser $user, ChildGroup $group)
    {
        if ($this->combinationCollGroupUsers === null) {
            $this->initGroupUsers();
        }

        if (!$this->getGroupUsers()->contains($user, $group)) {
            // only add it if the **same** object is not already associated
            $this->combinationCollGroupUsers->push($user, $group);
            $this->doAddGroupUser($user, $group);
        }
     ...
     * @param ChildUser $user
     */
    protected function doAddGroupUser(ChildGroup $group, ChildUser $user)
    {
        ...
        $userGroup->setGroupRole($this);

        $this->addUserRole($userGroup);

        // set the back reference to this object directly as using provided method either results
        // in endless loop or in multiple relations
        if ($group->isUserGroupRolesLoaded()) {
            $group->initUserGroupRoles();
            $group->getUserGroupRoles()->push($user, $this);
        } elseif (!$group->getUserGroupRoles()->contains($user, $this)) {
            $group->getUserGroupRoles()->push($user, $this);
        }

        // set the back reference to this object directly as using provided method either results
        // in endless loop or in multiple relations
        if ($user->isGroupGroupRolesLoaded()) {
            $user->initGroupGroupRoles();
            $user->getGroupGroupRoles()->push($group, $this);
        } elseif (!$user->getGroupGroupRoles()->contains($group, $this)) {
            $user->getGroupGroupRoles()->push($group, $this);
        }
    ...
    /**
     * Remove group, user of this object
     * through the usergroup cross reference table.
     * 
     * @param ChildGroup $group, 
     * @param ChildUser $user
     * @return ChildRole The current object (for fluent API support)
     */
    public function removeGroupUser(ChildGroup $group, ChildUser $user)
    {
        if ($this->getGroupUsers()->contains($group, $user)) { $userGroup = new ChildUserGroup();

            $userGroup->setGroup($group);
            if ($group->isUserGroupRolesLoaded()) {
                //remove the back reference if available
                $group->getUserGroupRoles()->removeObject($user, $this);
            }

            $userGroup->setUser($user);
            if ($user->isGroupGroupRolesLoaded()) {
                //remove the back reference if available
                $user->getGroupGroupRoles()->removeObject($group, $this);
            }

            $userGroup->setGroupRole($this);
            $this->removeUserRole(clone $userGroup);
            $userGroup->clear();

            $this->combinationCollGroupUsers->remove($this->combinationCollGroupUsers->search($group, $user));
            
            if (null === $this->combinationCollGroupUsersScheduledForDeletion) {
                $this->combinationCollGroupUsersScheduledForDeletion = clone $this->combinationCollGroupUsers;
                $this->combinationCollGroupUsersScheduledForDeletion->clear();
            }

            $this->combinationCollGroupUsersScheduledForDeletion->push($group, $user);
        }
    ...
    {
        if (null !== $this->aroleInGroup) {
            $this->aroleInGroup->removeGroupRole($this);
        }
        $this->id = null;
        $this->groupid = null;
        $this->role = null;
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->combinationCollGroupUsers) {
                foreach ($this->combinationCollGroupUsers as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
        ...
        $this->collUserRoles = null;
        $this->combinationCollGroupUsers = null;
        $this->aroleInGroup = null;
    }
