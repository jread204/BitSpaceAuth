...
abstract class Group implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\aceAuth\\Map\\GroupTableMap';
    ...
    /**
     * Initializes internal state of aceAuth\Base\Group object.
     */
     ...
     * @return $this|\aceAuth\Group The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : GroupTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\aceAuth\\Group'), 0, $e);
        }
    }
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collGroupRoles) {
                ...
                $result[$key] = $this->collGroupRoles->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\aceAuth\Group The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \aceAuth\Group (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \aceAuth\Group Clone of current object.
     * @throws PropelException
     */
    ...
    public function initGroupRoles($overrideExisting = true)
    {
        ...
        $this->collGroupRoles = new ObjectCollection();
        $this->collGroupRoles->setModel('\aceAuth\Role');
    }
     ...
     * @throws PropelException
     */
    public function getGroupRoles(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collGroupRoles || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collGroupRoles) {
                ...
                $collGroupRoles = ChildRoleQuery::create(null, $criteria)
                    ->filterByroleInGroup($this)
                    ->find($con);
     ...
     *
     * @param      Collection $groupRoles A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildGroup The current object (for fluent API support)
     */
    public function setGroupRoles(Collection $groupRoles, ConnectionInterface $con = null)
    {
        /** @var ChildRole[] $groupRolesToDelete */
        $groupRolesToDelete = $this->getGroupRoles(new Criteria(), $con)->diff($groupRoles);

        
        $this->groupRolesScheduledForDeletion = $groupRolesToDelete;

        foreach ($groupRolesToDelete as $groupRoleRemoved) {
            $groupRoleRemoved->setroleInGroup(null);
        }
     ...
     * @throws PropelException
     */
    public function countGroupRoles(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collGroupRolesPartial && !$this->isNew();
        if (null === $this->collGroupRoles || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collGroupRoles) {
     ...
     * @return $this|\aceAuth\Group The current object (for fluent API support)
     */
    public function addGroupRole(ChildRole $l)
    {
        if ($this->collGroupRoles === null) {
            $this->initGroupRoles();
            $this->collGroupRolesPartial = true;
        ...
        if (!$this->collGroupRoles->contains($l)) {
    ...
    /**
     * @param ChildRole $groupRole The ChildRole object to add.
     */
    protected function doAddGroupRole(ChildRole $groupRole)
    {
        $this->collGroupRoles[]= $groupRole;
        $groupRole->setroleInGroup($this);
    }

    /**
     * @param  ChildRole $groupRole The ChildRole object to remove.
     * @return $this|ChildGroup The current object (for fluent API support)
     */
    public function removeGroupRole(ChildRole $groupRole)
    {
        if ($this->getGroupRoles()->contains($groupRole)) {
            $pos = $this->collGroupRoles->search($groupRole);
            $this->collGroupRoles->remove($pos);
    ...
    public function initUsers()
    {
        ...
        $this->collUsers->setModel('\aceAuth\User');
    }
